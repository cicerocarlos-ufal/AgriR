...
anova.sp2.rbd <-
function(factor1, factor2, block, resp, quali=c(TRUE,TRUE), fac.fix="fac.A", fac.names=c('fac.A','fac.B'),graph=TRUE, graph.type="barplotb", graph.col="skyblue", xlab="Treatments", ylab="Variable", graph.den=2, mcomp='tukey', nl=FALSE, hvar='bartlett',sigT=0.05, sigF=0.05) 
{
  cat('------------------------------------------------------------------------\nLegend:\n')
  ...
  print(tab)
#-------------------CV-------------------
cv1=round(sqrt(tab[3,3])/mean(resp)*100,2)
cv1=data.frame(cv1,c("%"))
colnames(cv1)=c("","")
rownames(cv1)=c("Coefficient of Variance (Plot)")

cv2=round(sqrt(tab[6,3])/mean(resp)*100,2)
...
test[[i]]=tukey(resp,fatores[,i],as.numeric(tab[3*i,1]),as.numeric(tab[3*i,2]),sigT, graph, graph.type=graph.type, graph.col=graph.col, graph.den=graph.den,xlab=xlab,ylab=ylab)
                    }
  if(mcomp=='duncan'){

test[[i]]=duncan(resp,fatores[,i],as.numeric(tab[3*i,1]),as.numeric(tab[3*i,2]), sigT, graph, graph.type=graph.type, graph.col=graph.col, graph.den=graph.den,xlab=xlab,ylab=ylab)
                    }
  if(mcomp=='lsd'){
test[[i]]=lsd   (resp,fatores[,i],as.numeric(tab[3*i,1]),as.numeric(tab[3*i,2]), sigT, graph, graph.type=graph.type, graph.col=graph.col, graph.den=graph.den,xlab=xlab,ylab=ylab)
                    }
  if(mcomp=='scottknott'){
test[[i]]=scottknott(resp,fatores[,i],as.numeric(tab[3*i,1]),as.numeric(tab[3*i,2]),sigT, graph, graph.type=graph.type, graph.col=graph.col, graph.den=graph.den,xlab=xlab,ylab=ylab)  
                  }
  if(mcomp=='snk'){
test[[i]]=snk(resp,fatores[,i],as.numeric(tab[3*i,1]),as.numeric(tab[3*i,2]),sigT, graph, graph.type=graph.type, graph.col=graph.col, graph.den=graph.den,xlab=xlab,ylab=ylab)  
                  }

  if (mcomp=='scheffe') {
test[[i]]=scheffe(resp,fatores[,i],as.numeric(tab[3*i,1]),as.numeric(tab[3*i,3]),as.numeric(tab[3*i-2,4]), sigF, graph, graph.type=graph.type, graph.col=graph.col, graph.den=graph.den,xlab=xlab,ylab=ylab)
}
 }
...
for(i in 1:nv2) {
  ...
  if(quali[1]==TRUE) {
    ...
    if(mcomp=='tukey'){
      test[[i]]=tukey(resp[fatores[,2]==l2[i]], fatores[,1][fatores[,2]==l2[i]], as.numeric(tab.f1f2[nv2+1,1]),as.numeric(tab.f1f2[nv2+1,2]),sigT, graph, graph.type=graph.type, graph.col=graph.col, graph.den=graph.den,xlab=xlab,ylab=ylab)
                      }

  if(mcomp=='duncan'){
    test[[i]]=duncan(resp[fatores[,2]==l2[i]],fatores[,1][fatores[,2]==l2[i]],as.numeric(tab.f1f2[nv2+1,1]),as.numeric(tab.f1f2[nv2+1,2]),sigT, graph, graph.type=graph.type, graph.col=graph.col, graph.den=graph.den,xlab=xlab,ylab=ylab)
                    }

  if(mcomp=='lsd'){
    test[[i]]=lsd(resp[fatores[,2]==l2[i]],fatores[,1][fatores[,2]==l2[i]],as.numeric(tab.f1f2[nv2+1,1]),as.numeric(tab.f1f2[nv2+1,2]),sigT, graph, graph.type=graph.type, graph.col=graph.col, graph.den=graph.den,xlab=xlab,ylab=ylab)
                    }

  if(mcomp=='scottknott'){
    test[[i]]=scottknott(resp[fatores[,2]==l2[i]],fatores[,1][fatores[,2]==l2[i]],as.numeric(tab.f1f2[nv2+1,1]),as.numeric(tab.f1f2[nv2+1,2]),sigT, graph, graph.type=graph.type, graph.col=graph.col, graph.den=graph.den,xlab=xlab,ylab=ylab)
                    }

  if(mcomp=='snk'){
    snk(resp[fatores[,2]==l2[i]],fatores[,1][fatores[,2]==l2[i]],as.numeric(tab.f1f2[nv2+1,1]),as.numeric(tab.f1f2[nv2+1,2]),sigT, graph, graph.type=graph.type, graph.col=graph.col, graph.den=graph.den,xlab=xlab,ylab=ylab)
                   }
       }
...
if (quali[1]==TRUE && quali[2]==TRUE && tab[5,5]> sigF){return(list(Mcomp=test))}
if (quali[1]==FALSE && quali[2]==TRUE && tab[5,5]> sigF){return(list(Reg=reg[1], Mcomp=test[2]))}
if (quali[1]==TRUE && quali[2]==FALSE && tab[5,5]> sigF){return(list(Mcomp=test[1], Reg=reg[2]))}
if (quali[1]==FALSE && quali[2]==FALSE && tab[5,5] > sigF){return(list(Reg=reg))}
#----------------Return com interações----------------------------
if (quali[1]==TRUE && quali[2]==TRUE && tab[5,5] <=sigF){return(list(Mcomp=test))}
if (quali[1]==FALSE && quali[2]==TRUE && tab[5,5]){return(list(Reg=reg, Mcomp=test))}
if (quali[1]==TRUE && quali[2]==FALSE && tab[5,5]){return(list(Mcomp=test, Reg=reg))}
if (quali[1]==FALSE && quali[2]==FALSE && tab[5,5]){return(list(Reg=reg))}
