...
#' @export
scheffe <-
function (y, trt, DFerror, MSerror, Fc, alpha=0.05, graph, graph.col, graph.den, graph.type, xlab=xlab, ylab=ylab, group=TRUE,main = NULL,console=FALSE)
{
...
#Obter as letras
scheffe=data.frame("scheffe"=groups)
#rownames(scheffe)=output[,1]
scheffe=scheffe[order(rownames(scheffe)),]
scheffe=scheffe[,2]

#Juntar as informações

table.graph=cbind(medias,desv.pad,se.sup,se.inf,scheffe)


if(graph==TRUE){

if(graph.type=="boxplotb") boxplotb(y,trt,table.graph,graph.col=graph.col,xlab=xlab,ylab=ylab)

if(graph.type=="barplotb") barplotb(table.graph, graph.col=graph.col, graph.den=graph.den, xlab=xlab, ylab=ylab)

if(graph.type=="barplotg") barplotg(table.graph, graph.col=graph.col,xlab=xlab,ylab=ylab)

if(graph.type=="pointplotg") pointplotg(table.graph,graph.col=graph.col,xlab=xlab,ylab=ylab)
