...
#' @export
duncan <- function (y, trt, DFerror, SSerror, alpha = 0.05, graph, graph.col, graph.den, graph.type, xlab=xlab, ylab=ylab, group = TRUE,  main = NULL){
...
#Obter as letras
duncan=data.frame("mcomp"=output[,3])
rownames(duncan)=output[,1]
duncan=duncan[order(rownames(duncan)),]

#Juntar as informações

table.graph=cbind(medias,desv.pad,se.sup,se.inf,duncan)
