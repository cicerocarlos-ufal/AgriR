barplotg <-function(table.graph, xlab="", ylab="", title="", caption="", graph.col="skyblue", barcolor="orange", main=""){
...
p = ggplot(table.graph) +
    geom_bar( aes(x=rownames(table.graph), y=table.graph[,1]), stat="identity", fill=graph.col, alpha=0.7) +
    ...
    geom_text(aes(label = letras.tukey, x=rownames(table.graph), y=yl), size=5)
