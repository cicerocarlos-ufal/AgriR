...
#' @export
des.fat2.rbd <- function(factor1, factor2, rep, col.1="gray",col.2="white", graph.col=TRUE,fac.names=c('F1','F2'), save=FALSE){
...
mtr <- sample(trt.coul, t , replace = FALSE)

block <- c(rep(1, t))

for (y in 2:r) {
block <- c(block, rep(y, t))
mtr <- c(mtr, sample(trt.coul, t, replace = FALSE))
}
...
text(1,18, txt.factors, adj=0.5, cex=1, pos=2)

#text block
bl = paste(c("Block-"),block[i])
text(2, bl, adj=1, cex=1, pos=3)

}

if (save==TRUE){ dev.copy(tiff, "des.fat2.rdb.tiff", width=200*r, height=60*f1*f2, res=100)
dev.off() 
}
...
book=data.frame(factor1=y.f1, factor2=y.f2, Block=rept)
